...
from evo_sdk.inference import InferenceParams,RecurrentInferenceParams,HyenaInferenceEngine
from evo_sdk.layers import (Embedding,ParallelGatedMLP, RMSNorm,VocabParallelEmbedding)
from evo_sdk.StripedHyenaPreTrainedModel import \
    StripedHyenaPreTrainedModel
from evo_sdk.utils import column_split, dotdict, print_rank_0
...
try:
    from evo_sdk.positional_embeddings import swap_mha_rope
except ImportError:
...
class ParallelHyenaFilter(nn.Module):
    ...
    def forward(self, u, inference_params=None, padding_mask=None, *args, **kwargs):
        # u.shape -> [batch,sequence,embedding*3], padding_mask.shape ->[batch,sequence]
        if (
    ...
    def parallel_forward(self, u, inference_params=None, padding_mask=None):
        ...
        )  # z_pre.shape -> [batch,embedding*3,sequence]
        if inference_params:
            inference_params.fir_state_dict[self.layer_idx] = fir_state

        if self.h is None:
            h, filter_dtype, poles, residues = self.compute_filter(L, u.device)
            # h.shape -> [1,embedding,sequence],filter_dtype=torch.float32, poles/residues ->[self.num_systems, self.state_size, 1]
        else:
            h = self.h
        ...
        y = self.engine.parallel_iir(
            ...
            self.D,  # D.shape -> [hidden_size]
            L,  # sequence
            t=self.t,  # t.shape -> [1,1,sequence]
            poles=self.poles,  # [self.num_systems, self.state_size, 1]
            residues=self.residues,  # [self.num_systems, self.state_size, 1]
            dims=dims,
            inference_params=inference_params,
...
class ParallelGatedConvBlock(nn.Module):
    def __init__(self, config, layer_idx) -> None:
        ...
        if type(dtype) == str:
            dtype = (
                torch.float16
                if dtype == "ft16"
                else torch.bfloat16 if dtype == "bf16" else torch.float32
            )
        if type(mlp_dtype) == str:
            mlp_dtype = (
                torch.float16
                if dtype == "ft16"
                else torch.float32 if dtype == "ft32" else torch.bfloat16
            )
    ...
    def forward(self, u, inference_params=None, padding_mask=None, *args, **kwargs):
        # u.shape -> [batch,sequence,embedding], padding_mask.shape ->[batch,sequence]
        z = self.proj_norm_fn(u)  # z.shape -> [batch,sequence,embedding*3],
...
class StripedHyena(nn.Module):
    ...
    def forward(self, x, inference_params_dict=None, padding_mask=None):
        ...
        x = self.norm(x)
        if self.config.unembedding==True:
            x = self.unembed.unembed(x)
        return x, inference_params_dict_out

    def stateful_forward(self, x, inference_params_dict=None):#TODO: add checkpoint here
    ...
    def stateless_forward(self, x, padding_mask=None):
        # X.shape -> [batch,sequence], padding_mask.shape ->[batch,sequence]
        if type(padding_mask) == torch.Tensor:
            x = x * padding_mask[..., None]
        for _, block in enumerate(self.blocks):
            # x, _ = checkpoint(lambda x:(block(x, inference_params=None, padding_mask=padding_mask),None),x)
