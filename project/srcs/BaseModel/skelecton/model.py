...
from project.srcs.BaseModel.inference.cache import InferenceParams, RecurrentInferenceParams
from project.srcs.BaseModel.inference.engine import HyenaInferenceEngine
from project.srcs.BaseModel.skelecton.layers import ParallelGatedMLP, RMSNorm, VocabParallelEmbedding,Embedding
from project.utils.utils import column_split, print_rank_0
...
class StripedHyena(nn.Module):
    ...
    def precompute_filters(self, L, device):
        for block in self.blocks:
            if type(block) == ParallelGatedConvBlock and type(block.filter) == ParallelHyenaFilter:
